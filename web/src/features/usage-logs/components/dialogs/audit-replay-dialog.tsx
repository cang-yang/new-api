...
export function ReplayPreviewDetails(props: { preview: AuditReplayPreview }) {
  ...
  return (
    <div className="space-y-3 text-xs">
      <div className="grid gap-1 rounded-md border p-3 font-mono">
      ...
      {bodySummary && (
        <div>
          ...
          <pre className="bg-muted/35 max-h-44 overflow-auto whitespace-pre-wrap break-words rounded-md border p-3 font-mono text-[11px]">
            {bodySummary}
          </pre>
        </div>
      )}
    ...
    <div className="space-y-2 rounded-md border p-3 text-xs">
      ...
      {props.result.response_body && (
        <div>
          ...
          <pre className="bg-muted/35 max-h-40 overflow-auto whitespace-pre-wrap break-words rounded-md border p-2 font-mono text-[11px]">
      ...
      <AlertDialogContent className="max-h-[85dvh] overflow-y-auto sm:max-w-2xl">
        ...
        <div className="flex gap-2">
          {(["exact_upstream", "client_level"] as const).map((mode) => (
            ...
            >
              {t(mode === "exact_upstream" ? "Exact upstream" : "Client level")}
            </Button>
          ))}
        </div>
        {previewContent}
